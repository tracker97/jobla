...
braintree.Configuration.configure(braintree.Environment.Sandbox,
                                    merchant_id="jyvhycngd7xsrg5m",
                                    public_key="3ycgztf96djp64ws",
                                    private_key="4d851588384622f92059f746a0a4b224")
...
def edit_job(request, id):
    try:
        ...
        error = ''
        if request.method == 'POST':
            job_form = GigForm(request.POST, request.FILES, instance=job)
            if job_form.is_valid():
                job.save()
                return redirect('my_jobs')
            else:
